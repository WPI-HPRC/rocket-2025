...
void AttStateEstimator::applyGravUpdate(BLA::Matrix<3,1> a_b) {
    ...
    BLA::Matrix<3,1> bias = {
        x_min(AttKFInds::ab_x),
        x_min(AttKFInds::ab_y),
        x_min(AttKFInds::ab_z)
    };
    ...
    BLA::Matrix<3,13> K = P_min * BLA::MatrixTranspose<BLA::Matrix<3,13>>(H_grav) / (H_grav * P_min * BLA::MatrixTranspose<BLA::Matrix<3,13>>(H_grav) + R_grav);
